...
class Config {
  ...
  constructor ( name, options = {} ) {
    ...
    else this.path = path.resolve(__dirname, '..', '..', '..', 'config')
    ...
    this.loadFiles()
    if ( options.package !== false ) this.loadPackage()
  ...
  initConfig(){
    this.config = {}
    this.set(this.path_base_key, this.path)
    return this
  }
  ...
  setLocalPath(key, ...dirs) {
    this.set(`path.${key}`, path.join(this.path, ...dirs))
  }
