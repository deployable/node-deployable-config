...
module.exports = class Config {
  ...
  constructor ( name, options = {} ) {
    ...
    this.label = _.defaultTo(options.label, this.env())

    // A file to read from, normally the label
    this.file = _.defaultTo(options.file, `${this.label}.yml`)
    ...
    this.path = _.defaultTo( options.path, path.resolve( path.join(__dirname, '..', '..', 'config') ))
    ...
    if (options.package) this.set('package', options.package)
  }

  // Read a yaml `file` from `path`
  loadFile ( config_path, file ) {
    let file_path = path.join( config_path, file )
    let config = {}
    //try {
    ...
    //}
    debug('loadFile read config', file_path, config)
    this.loaded = true
    return config
  }
  ...
  loadFiles () {
    let config = {}
    if ( fs.existsSync(path.join(this.path, 'default.json')) ) config = _.merge(config, this.loadFile(this.path, 'default.json') )
    if ( fs.existsSync(path.join(this.path, 'default.yml')) )  config = _.merge(config, this.loadFile(this.path, 'default.yml') )
    if ( fs.existsSync(path.join(this.path, 'default.yaml')) ) config = _.merge(config, this.loadFile(this.path, 'default.yaml') )
    if ( fs.existsSync(path.join(this.path, this.file)) )      config = _.merge(config, this.loadFile(this.path, this.file) )
    debug('loadFiles merged config', this.path, this.file, config)
    if ( ! this.loaded ) throw new ConfigError(`No files were loaded from ${this.path}`)
    this.config = config
  }
  ...
  get (key) {
    ...
    if ( ! _.hasIn(this._config, key) ) throw new ConfigKeyError('Unknown config key', { key: key, status: 404 })
