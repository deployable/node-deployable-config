...
class Config {
  ...
  constructor ( name, options = {} ) {
    ...
    // A file to read from, normally the {label}.yaml
  ...
  readFile ( file_path ) {
    ...
    try {
      ...
      if ( extension === '.json' ) {
        ...
        debug('reading yaml file', file_path)
  ...
  loadFile( file_path, options = {} ){
    ...
    let config = this.readFile(file_path)
  ...
  loadFiles () {
    this.initConfig()
    this.loadFile(this.configPath('default.json'))
    this.loadFile(this.configPath('default.yml'))
    this.loadFile(this.configPath('default.yaml'))
